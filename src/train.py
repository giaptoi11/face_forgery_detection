...
def train(config, model, optimizer, train_loader, valid_loader, scheduler, save_dir):
    max_accuracy = 0
    global_step = 0
    ...
    # checkpoint
    if config.resume > -1:
        checkpoint = torch.load(os.path.join(save_dir, 'models_params_{}.tar'.format(config.resume)),
                                map_location='cuda:{}'.format(torch.cuda.current_device()))
        model.load_state_dict(checkpoint['model_state_dict'])
        optimizer.load_state_dict((checkpoint['optimizer_state_dict']))

    for epoch in range(config.resume+1, config.scheduler.total_epochs):
        # train part
        ...
        print(f'Epoch [{epoch+1:0>3}/{config.scheduler.total_epochs:0>3}], '
...
if __name__ == '__main__':
    ...
    config = load_config('src/configs/train_config.yaml')
    ...
    device = torch.device("{}".format(config.device) if torch.cuda.is_available() else "cpu")
    ...
    logging.info(config.save_dir)
    log.info('model dir:' + config.save_dir)

    train_loader = dataloader.get_dataloader(
        config.dataset.root,
        dataset_name=config.dataset.name,
        split_json=os.path.join(config.dataset.root, config.dataset.train_split),
        batch_size=32,
        phase="train"
    )
    ...
    test_loader = dataloader.get_dataloader(
        config.dataset.root,
        dataset_name=config.dataset.name,
        split_json=os.path.join(config.dataset.root, config.dataset.test_split),
        batch_size=1,
        phase="test"
    )
    ...
    train(config, model,optimizer,train_loader,test_loader,scheduler,save_dir)
